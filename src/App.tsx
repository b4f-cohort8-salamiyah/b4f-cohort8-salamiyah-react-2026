    ...
    <div>
      ...
      <main className="container">
        ...
        <section className="people-summary">
          {usersWithTaskCounts.map(function (item) {
            return (
              <PersonSummary
                key={item.user.id}
                name={item.user.name}
            ...
            return (
              <button
                ...
                className={`filter-button ${
                  selectedUserId === item.user.id ? "active" : ""
                }`}
                onClick={() => handleSelectUser(item.user.id)}
              >
                {item.user.name} ({item.taskCount})
              </button>
            );
          })}
        </section>

        <section className="toggle-section" style={{ margin: "1rem 0" }}>
          <button className="filter-button" onClick={handleToggleTasks}>
